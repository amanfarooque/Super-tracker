            ...
            if not friends_df.empty:
                ...
                if friend_to_chat:
                    ...
                    for _, row in chat_df.iterrows():
                        if row['sender'] == st.session_state.user:
                            st.markdown(f"<div style='text-align: right; color: #4CAF50;'><b>You:</b> {row['content']}</div>", unsafe_allow_html=True)
                        else:
                            st.markdown(f"<div style='text-align: left; color: #2196F3;'><b>{row['sender']}:</b> {row['content']}</div>", unsafe_allow_html=True)
            else:
                st.info("Add some friends in the first tab to start chatting!")
            conn.close()
